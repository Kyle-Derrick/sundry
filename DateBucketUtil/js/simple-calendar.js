...
//      '2016-5-5': '上学'
  ...
  update(month = this.tmonth, year = this.tyear) {
    ...
    this.updateTimePeriod(year, month);
  }
  ...
  updateTimePeriod(year, month) {
    var that = this;
    var options = this._options;
    if (options.dateBucketUtil) {
      var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
      var currentmonth = month - 1;
      //获取时间段数据
      var dateBucketUtil = options.dateBucketUtil;
      daysElement.forEach(function (v, i) {

        var day = +v.querySelector('.day').innerHTML;
        if (day === 1) currentmonth++;

        var item = dateBucketUtil.search(new Date(year, currentmonth - 1, day)).value;
        var child = v.querySelector(".time-period-div");
        var reminder = v.querySelector('.lunar-day');
        if (item) {
          v.classList.add("time-period");
          var newNode;
          if(!child){
            newNode = document.createElement("div");
            newNode.className = "time-period-div";
          }else{
            newNode = child;
          }
          newNode.innerHTML = "<div>挂牌价:"+
              item.listedPrice+"</div><div>结算价:"+
              item.settlementPrice+"</div><div>销售价:"+
              item.sellPrice+"</div>";
          if(!child){
            v.appendChild(newNode);
          }

          reminder.innerText = "销售价:"+item.sellPrice;
        } else {
          v.classList.remove("time-period");
          if(child)
            v.removeChild(child);
  ...
  updateEvent() {
    ...
    daysElement.forEach(function (v, i) {
      v.onmouseover = function (e) {
        ...
        var n = this.querySelector(".time-period-div");
        if(n){
          n.style.display = "block ";
          n.style.cssText = "display: block !important;";
        }
      };
      v.onmouseout = function (e) {
